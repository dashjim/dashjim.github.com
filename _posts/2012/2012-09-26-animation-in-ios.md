...
这里再强调关于动画的两个重要的点:一个是中间状态的插值计算Interpolation,二是什么时间对应什么状态Timing； 有时候插值计算也依赖于Timing。
如果状态是一维空间的值(比如透明度)，那么插值计算的结果必然再起点值和终点值之间,如果状态是二维空间的值(比如position)，那么一般情况下插值得到的点会落在起点和终点之间的线段上。
#### 1.CABasicAnimation
